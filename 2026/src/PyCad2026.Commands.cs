...
namespace PYLOAD2026R
{
    public partial class PyCad2026
    {
        ...
        {
            MethodInfo getMi = typeof(ZwApp).GetMethod("GetSystemVariable", BindingFlags.Public | BindingFlags.Static);
            MethodInfo setMi = typeof(ZwApp).GetMethod("SetSystemVariable", BindingFlags.Public | BindingFlags.Static);
            return getMi != null && setMi != null;
        }
        ...
        public void RunCommands(IList commandTexts)
        {
            ...
            LogShell("in", "command-batch", text.TrimEnd());
        ...
        public void ZoomAll()
        {
            Command(new ArrayList { "_.ZOOM", "_A" });
        }

        public void ZoomPrevious()
        {
            Command(new ArrayList { "_.ZOOM", "_P" });
        }
        ...
        public void ZoomWindow(double x1, double y1, double z1, double x2, double y2, double z2)
        {
            Command(new ArrayList
            {
                "_.ZOOM",
                "_W",
                FormatPoint(x1, y1, z1),
                FormatPoint(x2, y2, z2)
            });
        }
        ...
        public void ZoomObject(ObjectId entityId)
        {
            ...
            RunLisp(string.Format(CultureInfo.InvariantCulture, "(command \"_.ZOOM\" \"_O\" (handent \"{0}\"))", EscapeLispString(handle)));
        }
        ...
        public void OpenScript(string scriptFilePath)
        {
            ...
            string full = Path.GetFullPath(scriptFilePath);
        ...
        public void LoadLispFile(string lspFilePath)
        {
            ...
            string full = Path.GetFullPath(lspFilePath);
            RunCommand("(load \"" + EscapeLispString(full) + "\")");
        }
        ...
        public void CallLisp(string functionName, IList args)
        {
            ...
            sb.Append("(").Append(functionName.Trim());
            if (args != null)
            {
                foreach (object raw in args)
                {
                    sb.Append(" ").Append(FormatLispAtom(raw));
                }
            }
            sb.Append(")");
            RunLisp(sb.ToString());
        ...
        public void Command(IList commandArgs)
        {
            ...
            foreach (object raw in commandArgs)
            {
                ...
                sb.Append(" \"").Append(EscapeLispString(arg)).Append("\"");
            }
            sb.Append(")");
            RunLisp(sb.ToString());
        }
        ...
        private static string FormatLispAtom(object value)
        {
            ...
            if (value is string)
            {
                return "\"" + EscapeLispString((string)value) + "\"";
            }

            if (value is char)
            {
                return "\"" + EscapeLispString(value.ToString()) + "\"";
            }
